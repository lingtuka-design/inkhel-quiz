...
function SeasonCard({ season, currentMonth }: { season: Season; currentMonth: Month | null | undefined }) {
  ...
  const { data: ranking } = useQuery({
    ...
    queryFn: () => getSeasonRanking(season.id),
  })
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-12 sm:px-6">
      ...
      <div className="space-y-8">
        {seasons.map((season) => (
          <SeasonCard key={season.id} season={season} currentMonth={currentMonth} />
        ))}
      </div>
    </div>
  )
